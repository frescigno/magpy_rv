...
Made specifically for GP_solar.py
Contains:
    Body of MCMC
...
def get_model(model_name, time, model_par):
    '''
    Parameters
    ----------
    model_name : string
    ...
    if model_name.startswith("off") or model_name.startswith("Off"):
        model = gp.Offset(time, model_par)
        model_y = model.model()
    if model_name.startswith("kepl") or model_name.startswith("Kepl"):
        model = gp.Keplerian(time, model_par)
        model_y = model.model()
...
class MCMC:
    ...
    def __init__(self, t, rv, rv_err, hparam0, kernel_name, model_par0, model_name, prior_list, numb_chains=100):
        ...
        if self.model_name.startswith("kepl") or self.model_name.startswith("Kepl"):
            self.single_modpar0[2], self.single_modpar0[3], self.modpar_err[2], self.modpar_err[3] = aux.to_SkCk(self.single_modpar0[2], self.single_modpar0[3], self.modpar_err[2], self.modpar_err[3])
        ...
        for chain in range(self.numb_chains):
            ...
            model_par_chain = gp.Model_Par_Creator.create(self.model_name)
    ...
    def split_step(self, n_splits=2, a=1.25, Rstar=None, Mstar=None):
        ...
        for split in range(n_splits):
            ...
            for chain in range(N_chains1):
                ...
                # Orbit check
                # If Rstar and Mstar are given do it, else always say it's ok
                if Rstar is not None and Mstar is not None:
                    orbit_check = aux.star_cross(Xk_new_mod[2], Xk_new_mod[3], Rstar, Xk_new_mod[0], Mstar)
                else:
                    orbit_check = True
                
                # 
                if self.model_name.startswith("kepl") or self.model_name.startswith("Kepl"):
                    while (np.min(Xk_new) < 0) or (Xk_new_mod[0]<0.) or (Xk_new_mod[1]<0.) or (Xk_new_mod[4]<0.) or ((Xk_new_mod[2]**2 + Xk_new_mod[3]**2)>1) or (not orbit_check):
                        # Compute the step and apply it
                        # The random number is the same within a chain but different between chains
                        z = (np.random.rand()*(a-1) + 1)**2 / a
                        Xk_new = Xj + (S1_hp[chain] - Xj) * z
                    ...
                    #print("in else")
                    while (np.min(Xk_new) < 0) or (np.min(Xk_new_mod)< 0):
                        #print("in while")
                        # Compute the step and apply it
                        # The random number is the same within a chain but different between chains
                        z = (np.random.rand()*(a-1) + 1)**2 / a
    ...
    def compute(self):
        ...
        for chain in range(self.numb_chains):
            ...
            model_param = None
            model_param = gp.Model_Par_Creator.create(self.model_name)
            
            if not self.model_name.startswith("kepl") and not self.model_name.startswith("Kepl"):
            ...
            # Need to turn Sk and Ck back to ecc and omega
            if self.model_name.startswith("kepl") or self.model_name.startswith("Kepl"):
                ecc, omega = aux.to_ecc(self.modpar[chain][2], self.modpar[chain][3])
                #print(ecc, omega)
                model_param["P"] = gp.Parameter(value=self.modpar[chain][0], error=self.modpar_err[0], vary=self.modpar_vary[0])
                model_param["K"] = gp.Parameter(value=self.modpar[chain][1], error=self.modpar_err[1], vary=self.modpar_vary[1])
                model_param["ecc"] = gp.Parameter(value=ecc, vary=self.modpar_vary[2])
                model_param["omega"] = gp.Parameter(value=omega, vary=self.modpar_vary[3])
                model_param["t0"] = gp.Parameter(value=self.modpar[chain][4], error=self.modpar_err[4], vary=self.modpar_vary[4])
